...
function getRemoteImage( $uri )
{
    ...
    foreach ( $imgUrls as $imgUrl ) {
        ...
        //创建保存位置
        $savePath = $config[ 'savePath' ].date('Ym',time())."/";
        if ( !file_exists( $savePath ) ) {
            mkdir( "$savePath" , 0777 );
        }
        //写入文件
        //$tmpName = $savePath . rand( 1 , 10000 ) . time() . strrchr( $imgUrl , '.' );
		$tmpName = $savePath.date('Ymd_His',time())."_".rand(1,99999).strrchr( $imgUrl , '.' );
        try {
            $fp2 = @fopen( $tmpName , "a" );
            fwrite( $fp2 , $img );
            fclose( $fp2 );
			$tmpName_2 = "/".str_ireplace("../","",$tmpName);
            array_push( $tmpNames ,  $tmpName_2 );
        } catch ( Exception $e ) {
            array_push( $tmpNames , "error" );
        }
    }
