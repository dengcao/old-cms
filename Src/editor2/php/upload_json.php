...
$dir_get=$_GET['dir'];
if ($dir_get=="image"){$dir_get="img";}//更改image为img，邓草 2012-07-24

$php_path = dirname(__FILE__) . '/';
$php_url = dirname($_SERVER['PHP_SELF']) . '/';

//文件保存目录路径
$save_path = $php_path . '../../5300/upload/';
//文件保存目录URL
$save_url = $php_url . '../../5300/upload/';
//定义允许上传的文件扩展名
$ext_arr = array(
...
//最大文件大小
$max_size = WEB_UPLOAD_MAXSIZE*1024;
...
if (empty($_FILES) === false) {
	...
	//检查目录名
	$dir_name = empty($dir_get) ? 'img' : trim($dir_get);
	...
	$new_file_name = $path.date('Ymd_His',time())."_".rand(1,99999).'.' . $file_ext;
	//移动文件
	...
	if (move_uploaded_file($tmp_name, $file_path) == false){
		...
		if(WEB_ISWATERMARK!=0){//为图片添加水印			        
				    ...
				    $WaterMark->watermark($file_path);	//执行添加水印方法					
					}
		}
