...
CheckUserPopedom("channel");//检查已登陆管理员是否具备管理此页面的权限
global $conn;
$item_name_all="栏目";//定义项目总名称，比如栏目
$parentid_all=$_POST["parentid_all"]; //父分类总ID，比如产品=1，新闻=2，栏目=4等，具体看数据表
if (!$parentid_all){
...
switch ($action_app){
   case "saveadd";
	...
	if ($classname==""){
	   echo_js("alert('".$item_name_all."名称不能为空！');history.back();");
	   exit;
	   }
	...
	if ($result){
	       echo_js("alert('添加成功！');location.href=\"admin.php?action=channel&parentid_all=$parentid_all\";");
		   }else{
   ...
   case "saveedit";
	...
	$seo_description=trim($_POST["seo_description"]);
	if ($classid==""){
	   echo_js("alert('请选择修改的".$item_name_all."！');history.back();");
	   exit;
	   }
	if ($classname==""){
		echo_js("alert('".$item_name_all."名称不能为空！');history.back();");
	   exit;
		 ...
		 $result=mysql_query("update pl_class set classname='$classname',classcontents='$classcontents',imgurl='$imgurl',isout=$isout,outurl='$outurl',visible=$visible,seo_title='$seo_title',seo_keyword='$seo_keyword',seo_description='$seo_description',orderby=$orderby where classid=$classid",$conn);
	        if ($result){
	       echo_js("alert('修改成功！');location.href=\"admin.php?action=channel&parentid_all=$parentid_all\";");
		   }else{
	...
	mysql_close($conn);
	if ($result_del){
	       echo_js("alert('删除成功！');location.href=\"admin.php?action=channel&parentid_all=$parentid_all\";");
		   }else{
...
function ShowLink2(t){
	if(t==1){display_str="none";}else{display_str="";}
		for(i=1;i<=4;i++){
		  document.getElementById("Showmore"+i).style.display=display_str;
		}
	}
</script>
...
<form action="admin.php?action=channel&parentid_all=<?=$parentid_all?>" method="post" name="classadd" id="classadd">
<br>
<table cellSpacing="0" cellPadding="0" width="98%" align="center" border="0">
				<tr>
					<td>
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#f3f8ff">
                          ...
                          <tr>
								<td width="15%" height="37" align="right">上级<?=$item_name_all?>：</td>
								<td>
                                  <select name="parentid" id="parentid">
                                  <option value="<?=$parentid_all?>" selected="selected">作为一级分类</option>                                  <?php
							...
							<tr>
								<td width="15%" height="37" align="right"><?=$item_name_all?>名称：</td>
								<td>
								  <input name="classname" type="text"  id="classname" size="60" maxlength="255"  style="width:250px;">
									<font color="#ff0000">* </font></td>
							</tr>
                             <tr style="display:none;">
								<td width="15%" height="37" align="right"><?=$item_name_all?>图片：</td>
								<td>
		...
		<td><input type='text' size='30' maxlength='255' name='outurl' style="width:250px;">&nbsp;&nbsp;(为外部链接时填写)</td>
                            ...
                            <tr style="display:;" id="Showmore4">
								<td width="15%" height="37" align="right"><?=$item_name_all?>内容：</td>
								<td>
								  <textarea name="classcontents" style="width:800px;height:450px;visibility:hidden;"></textarea></td>
							</tr>
           <tr>
...
<form action="admin.php?action=channel&parentid_all=<?=$parentid_all?>" method="post" name="classedit" id="classedit">
<br>
<table cellSpacing="0" cellPadding="0" width="98%" align="center" border="0">
				<tr>
					<td>
		...
		<td><input type='text' size='30' maxlength='255' name='outurl' style="width:250px;" value="<?php echo $myrow['outurl'];?>">&nbsp;&nbsp;(为外部链接时填写)</td>
                            ...
                            <tr style="display:<?php if($myrow['isout']==1){echo "none";}?>;" id="Showmore1">
								<td width="15%" height="37" align="right">SEO标题：</td>
								<td>
								  <input name="seo_title" type="text"  id="seo_title" size="60" maxlength="255"  style="width:250px;" value="<?php echo $myrow['seo_title'];?>"></td>
							</tr>
                            <tr style="display:<?php if($myrow['isout']==1){echo "none";}?>;" id="Showmore2">
								<td width="15%" height="37" align="right">SEO关键词：</td>
								<td>
                            ...
                            <tr style="display:<?php if($myrow['isout']==1){echo "none";}?>;" id="Showmore3">
								<td width="15%" height="37" align="right">SEO描述：</td>
								<td>
                            ...
                            <tr style="display:<?php if($myrow['isout']==1){echo "none";}?>;" id="Showmore4">
								<td width="15%" height="37" align="right"><?=$item_name_all?>内容：</td>
								<td>
								  <textarea name="classcontents" style="width:800px;height:450px;visibility:hidden;"><?php echo htmlspecialchars($myrow['classcontents']);?></textarea></td>
							</tr>
                            <tr>
								<td height="37" align="right">显示顺序：</td>
							  <td>
...
function cf(t){
if(confirm("您确定删除【"+t+"】这个<?=$item_name_all?>吗？\n\n删除此<?=$item_name_all?>将同时删除其下级<?=$item_name_all?>。此操作将无法恢复，请慎重操作。")==true){
	return true;
	}else{
	return false;	
		}
...
<ul class="classlist">
<?php
	PinLuo_ViewClass($parentid_all,10,"admin.php?action=channel&parentid_all=$parentid_all&");
?>
</ul>
<div><br><br><br><a href='admin.php?action=channel&parentid_all=<?=$parentid_all?>&action_app=add' style="background:url(images/add_alt.gif) no-repeat;padding-left:20px;display:block;height:20px;font-weight:normal">添加分类</a>
<br><br /><br />
          <font style="color:#aaa; font-weight:normal">说明：( )内数字代表包含下属<?=$item_name_all?>，[ ]内数字代表排序号，“隐”代表隐藏<?=$item_name_all?>，“外”代表外部链接。</font>
          </div></td>
		  </tr>
		</table>			
