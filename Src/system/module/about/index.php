...
class about extends base{
	...
	public static function get_class_content($classid){//根据ID获取某分类的数据，成功则返回数组
		...
		$sql="select * from ".DB_PRE."class where parentid>0 and classid=".intval($classid);
				...
				$sql="select * from ".DB_PRE."class where parentid=4 order by orderby desc,classid LIMIT 0,1";
			}
...
//接收参数
$classid=safe_replace(safe_html($_GET["id"]));
$about=new about();
$class_arr=$about->get_class_content($classid);
...
template("index","about","about_".$classid."_".md5(array_to_string($_GET,$separator="_")));